__version__ = "1.1.3"
# Changelog
# 1.1.3 - No functional change; version bumped to match Fourier_params.py 1.1.3, which
#         consolidated the double-assignment config switches into single assignments
#         with True/False doc comments (no flag values changed).
# 1.1.2 - Fixed a positional-argument bug in sigma_shape_combined_Fm and
#         sigma_shape_by_age_grp_sw_combined_Fm: both took a trailing `age_grp=0`
#         parameter that was never read in either body (sigma_shape_combined_Fm never
#         referenced it; sigma_shape_by_age_grp_sw_combined_Fm immediately overwrote it
#         with a local `age_grp = 0` counter). The notebook calls both with 5 positional
#         args (sim, sigmaFm, snap, image_dir, save_image_file), so save_image_file was
#         silently binding to the dead age_grp slot instead of save_file, which always
#         kept its default True - meaning these two plots were always saved regardless
#         of the save_image_file config flag. Removed the unused age_grp parameter from
#         both signatures so save_file now receives the caller's value correctly.
# 1.1.1 - Stage 2: de-duplicated the sigma-shape polar-binning/Fourier-amplitude logic
#         that was repeated (near verbatim) across sigma_shape_Fm,
#         sigma_shape_by_age_grp_sw_combined_Fm and sigma_shape_combined_Fm. Extracted
#         three shared helpers: sigma_polar_grid, sigma_fourier_amplitude_phase and
#         sigma_amplitude_peak_within_1kpc. sigma_polar_grid now stores only the per-bin
#         sigma (LOS velocity dispersion) instead of an object array that also carried
#         each bin's raw x/y/vz sub-arrays - those were computed but never read by any
#         of the three callers (verified by search), so dropping them changes memory/
#         speed only, not any returned numeric value. All other math (radial/angular
#         binning, angle wrap-around, Fourier sums) is unchanged. File shrank from 2267
#         to 1944 lines.
# 1.1.0 - Removed dead code (apply_umask_old, fit_ellipses_to_density_old, bar_length,
#         sigma_shape_by_age_Fm, duplicate plot_sigma_amp_bar_ellip_timeline, plot_density_hist2d)
#         and an unused import. No behavior change to functions still in use.
...
from Fourier_params_v1_1_3 import *
...
# Bins star particles into (radial, angular) polar cells in the x-z plane
# (edge-on, bar end-on projection) and returns the LOS velocity dispersion
# (sigma) per cell. Shared by sigma_shape_Fm, sigma_shape_combined_Fm and
# sigma_shape_by_age_grp_sw_combined_Fm.
def sigma_polar_grid(x, y, z, vz):

    #Calculate the radius of each particle and its cylindrical angle phi
    Rs = np.hypot(x, z) # Hypotenuse of right angle triangle with x,z
    phis = np.arctan2(z, x) # in radians

    # Define bins for radius from a pre-defined bin_width in kpc
    ...
    # Find the radial bin index of each particle
    radial_bin_indices = np.digitize(Rs, bins=radial_bins) - 1

    # Find the angular bin index of each particle. arctan2 returns angles in
    # (-pi, pi], so wrap negative angles into [0, 2*pi) before digitizing and
    # keep whichever bin index actually falls in range.
    phi_bin_indices = np.digitize(phis, bins=phi_bins) - 1
    phis_wrapped = (phis + 2 * np.pi) % (2 * np.pi)
    phi_bin_indices_wrapped = np.digitize(phis_wrapped, bins=phi_bins) - 1

    num_radial_bins = len(radial_bins) - 1
    num_phi_bins = len(phi_bins) - 1

    # sigma_grid[i, j] = LOS velocity dispersion of stars in radial bin i,
    # angular bin j
    sigma_grid = np.empty((num_radial_bins, num_phi_bins))
    for i in range(num_radial_bins):
        ...
        for j in range(num_phi_bins):
            ...
            sigma_grid[i, j] = np.std(vz[inside_bin])

    # Drop the trailing bin edge so radial_bins/phi_bins line up with sigma_grid
    ...
    return radial_bins, phi_bins, sigma_grid


# Computes the amplitude and phase of Fourier moment Fm of a sigma_grid (from
# sigma_polar_grid) as a function of radius. Shared by sigma_shape_Fm,
# sigma_shape_combined_Fm and sigma_shape_by_age_grp_sw_combined_Fm.
def sigma_fourier_amplitude_phase(Fm, phi_bins, sigma_grid):
    ...
    for i in range(sigma_grid.shape[0]):
        aF = hypot(np.sum(sFp*sigma_grid[i,:]),np.sum(cFp*sigma_grid[i,:]))/np.sum(sigma_grid[i,:])
        phiF = (1/Fm) * degrees(arctan2(np.sum(sFp*sigma_grid[i,:]),np.sum(cFp*sigma_grid[i,:])))
    ...
    return aF_plot, phiF_plot


# Finds the amplitude peak (and its radius) within the inner 1 kpc of an
# amplitude profile. Shared by sigma_shape_combined_Fm and
# sigma_shape_by_age_grp_sw_combined_Fm.
def sigma_amplitude_peak_within_1kpc(aF_plot, radial_bins):
...
def sigma_shape_Fm(sim,Fm,snap,image_dir,age_grp=0,save_file=True):
    ...
    radial_bins, phi_bins, sigma_grid = sigma_polar_grid(x, y, z, vz)
    aF_plot, phiF_plot = sigma_fourier_amplitude_phase(Fm, phi_bins, sigma_grid)
...
def sigma_shape_by_age_grp_sw_combined_Fm(sim,sigmaFm,snap,image_dir,save_file=True):
    ...
    for grp in groups:
        ...
        for Fm in sigmaFm:
            aF_plot, phiF_plot = sigma_fourier_amplitude_phase(Fm, phi_bins, sigma_grid)
            aF_max, aF_max_R = sigma_amplitude_peak_within_1kpc(aF_plot, radial_bins)
...
def sigma_shape_combined_Fm(sim,sigmaFm,snap,image_dir,save_file=True):
